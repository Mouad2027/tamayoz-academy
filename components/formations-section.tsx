      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {formations.map((formation, index) => (
            ...
            >
              <div
                className={`premium-card bg-white p-8 rounded-xl h-full flex flex-col relative overflow-hidden border-t-4 border-${formation.color} shadow-lg hover:shadow-xl transition-all duration-300`}
              >
                {/* Formation Header */}
                <div className="mb-6 text-left">
                  <div
                    className={`p-3 rounded-xl bg-${formation.color}/10 inline-flex items-center justify-center mb-4`}
                  >
                ...
                <div className="mb-6 flex-grow">
                  <h4 className={`text-sm uppercase tracking-wider text-${formation.color} font-semibold mb-3`}>
                    Inclus dans ce programme
                  </h4>
                  <ul className="space-y-2">
                    {formation.features.map((feature, idx) => (
                      <li key={idx} className="flex items-start">
                        <CheckCircle className={`h-4 w-4 text-${formation.color} mr-2 mt-1 flex-shrink-0`} />
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div
                    className={`flex items-center space-x-2 p-3 rounded-lg bg-${formation.color}/5 border border-${formation.color}/20`}
                  >
                    <Users className={`h-5 w-5 text-${formation.color} flex-shrink-0`} />
                    <span className="text-sm text-slate-700 whitespace-nowrap font-medium">Présentiel</span>
                  </div>
                  <div
                    className={`flex items-center space-x-2 p-3 rounded-lg bg-${formation.color}/5 border border-${formation.color}/20`}
                  >
                    <Monitor className={`h-5 w-5 text-${formation.color} flex-shrink-0`} />
                    <span className="text-sm text-slate-700 whitespace-nowrap font-medium">En ligne</span>
                  </div>
                </div>
